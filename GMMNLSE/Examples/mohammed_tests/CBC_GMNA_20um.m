...
pump_power_forward  = 3; % W
pump_power_backward = 3; % W
...
gain_rate_eqn.t_rep = 10/48e6; % Assume 4 MHz here; s; the time required to finish a roundtrip (the inverse repetition rate of the pulse)
...
% fiber.betas = [8.853357718031649e6; 4884.295653269443; 0.018665078612606148; 4.1371072089738246e-5; -5.064353346238818e-8];

%% Setup initial conditions
tfwhm = 0.5; % ps
total_energy = 1; % nJ
seed_wavelength = 1030e-9; % m
num_modes = 1;
freq_shift = c/seed_wavelength*1e-12 - sim.f0;
Gaussian_pulse = build_MMgaussian(tfwhm, time_window, total_energy, num_modes, N,seed_wavelength,phis, {'ifft',freq_shift});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% add unpumped doped fiber
% sim.save_period = 0.01;
% gain_rate_eqn{1}.copump_power = 0; % W
% gain_rate_eqn{1}.counterpump_power = 0; % W

% fiber.L0 = 0.5;

% input_pulse1 = input_pulse{1}
% input_pulse1.fields = prop_output.fields.forward(:,:,end)

% prop_output1 = GMMNLSE_propagate(fiber, input_pulse1, sim, gain_rate_eqn{1});
% [prop_output1(:).N2] = 0;
% [prop_output1(:).lambda0] = sim.lambda0;
% save("C:\Users\ms2106\Heriot-Watt University Team Dropbox\RES_EPS_Lupo\Projects\UCARE\FibreLasers\simulations\GMNA\data_CBC.mat", ...
%                 "prop_output1", '-v7.3');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

[Strehl_ratio,dechirped_FWHM,transform_limited_FWHM,peak_power] = analyze_field( t,f,prop_output.fields.forward,'Treacy-t',pi/6,1e-6 );
